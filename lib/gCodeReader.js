 ...
 * Time: 7:31 AM
 *
 * Modified by Albert MN. - SimplyPrint 24/01/2020
 */
...
//        console.log("cura");
    ...
    var detectSlicer = function (gcode, getParamsFromSlicer = false) {
        if (gcode.match(/Slic3r/)) {
            slicer = "Slic3r";
            if (getParamsFromSlicer) {
            ...
            slicer = "makerbot";
            /*if (getParamsFromSlicer) { //is empty...
                getParamsFromMiracleGrue(gcode);
            }*/
        } else if (gcode.match(/ffslicer/)) {
            slicer = "Flash Forge";
        } else if (gcode.match(/Simplify3D/)) {
            slicer = "Simplify3D";
        }
    };
    ...
    // ***** PUBLIC *******
    return {
        loadFile: function (fileContent, kiriSliced = false, oldLoad = false) {
            model = [];
            z_heights = [];

            if (!kiriSliced) {
                //Not sliced through Kiri - let's check which slicer it actually is
                detectSlicer(oldLoad ? fileContent.target.result : fileContent, (typeof gCodeOptions.filamentDia === "undefined" || gCodeOptions.filamentDia == null || isNaN(gCodeOptions.filamentDia) || typeof gCodeOptions.nozzleDia === "undefined" || gCodeOptions.nozzleDia == null || isNaN(gCodeOptions.nozzleDia)));
            } else {
                slicer = "SimplyPrint";
            }

            lines = oldLoad ? fileContent.target.result.split(/\n/) : fileContent.split(/\n/);
            if (oldLoad) {
                fileContent.target.result = null;
            }
            //prepareGCode();
...
//            console.log(model);
        ...
        processAnalyzeModelDone: function (msg) {
            ...
            let density = gCodeOptions.filamentDensity;
